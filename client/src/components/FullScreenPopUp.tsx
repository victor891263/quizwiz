...
export default function FullScreenPopUp({children, isVisible, close, className}: {children: JSX.Element, isVisible: boolean, close: () => void, className?: string}) {
    return (
        <>
            <div className={'fixed top-0 left-0 w-screen h-screen z-10 transition-colors ' + (isVisible ? 'bg-gray-900/25 dark:bg-white/10 dark:backdrop-blur-sm' : 'pointer-events-none')}></div>
