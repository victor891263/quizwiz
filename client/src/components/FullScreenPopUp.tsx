        ...
        <>
            ...
            <div className={'fixed top-0 right-0 h-screen z-20 bg-white overflow-y-auto transition-transform ' + (isVisible ? '' : 'translate-x-full ') + (className || '')}>
                <div className='relative'>
                    <button onClick={close} className='absolute top-0 right-0 h-6 w-6 flex items-center justify-center secondary !p-0'><CrossIcon className={'shrink-0 w-4 h-4'} /></button>
                    {children}
                </div>
            </div>
        </>
